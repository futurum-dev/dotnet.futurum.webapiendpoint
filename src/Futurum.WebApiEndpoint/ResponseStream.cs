...
{
    public Result<ResponseStreamDto> Map(ResponseStream<TApiEndpoint> domain) =>
        new ResponseStreamDto(domain.Stream, domain.ContentType, domain.FileName, domain.FileLengthBytes).ToResultOk();
}