...
public class WebApiEndpointMiddlewareExecutorTests
{
    private readonly ITestOutputHelper _output;

    private const string ErrorMessage = "ERROR_MESSAGE";

    public WebApiEndpointMiddlewareExecutorTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    public class FailureMiddleware<TRequest, TResponse> : IWebApiEndpointMiddleware<TRequest, TResponse>
    {
        private readonly Action _action;
    ...
    public async Task when_ApiEndpoint_returns_failure_then_correct_error_returned()
    {
        ...
        var middlewares = new IWebApiEndpointMiddleware<Command, Response>[] {testMiddleware1, testMiddleware2};
        var middlewareExecutor = new WebApiEndpointMiddlewareExecutor<Command, Response>(middlewares);
